...
def create_model(cat_idxs = None):
    model = GradientBoostingRegressor(random_state=random.randint(3, 1000), n_estimators=12000, learning_rate=0.01)
    return model
...
for version in tqdm([0, 1, 2, 3, 4]):
    for city in tqdm(['hcm', 'hn']):
        ...
        path = f'../model/{city}/gbr/v{version}/'
        ...
        load_model = load(f"../model/{city}/gbr/v{version}/model.joblib")
