...
def create_model(cat_idxs = None):
    model = MLPRegressor(random_state=random.randint(3, 1000), max_iter = 5000)
    search_params =  {'learning_rate': ['constant', 'invscaling', 'adaptive']}
...
for version in tqdm([0, 1, 2, 3, 4]):
    for city in tqdm(['hcm', 'hn']):
        ...
        path = f'../model/{city}/mlp/v{version}/'
        ...
        load_model = load(f"../model/{city}/mlp/v{version}/model.joblib")
